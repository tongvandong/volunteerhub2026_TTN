...
const getNotificationTone = (type = "") => {
  ...
  if (
    ...
    return {
      icon: "check_circle",
      box: "bg-success-container text-success",
      dot: "bg-success",
    };
  }
  if (
    ...
    return {
      icon: "error",
      box: "bg-error-container text-error",
      dot: "bg-error",
    };
  }
  if (
    ...
    normalized.includes("understaffed")
  ) {
    return {
      icon: "warning",
      box: "bg-warning-container text-amber-700",
      dot: "bg-warning",
    };
  }
  if (
    ...
    return {
      icon: "how_to_reg",
      box: "bg-primary-container text-primary",
      dot: "bg-primary",
    };
  }
  ...
  return {
    icon: "notifications",
    box: "bg-surface-variant text-on-surface-variant",
    dot: "bg-primary",
  };
};
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-on-surface-variant">
            Theo dõi cập nhật mới từ sự kiện, đăng ký, xác minh và tài trợ.
          </p>
        </div>
      ...
      {notifications.length === 0 ? (
        ...
        <div className="space-y-3">
          {notifications.map((notif) => {
            ...
            const message =
              notif.message && notif.message !== title ? notif.message : "";
            ...
            return (
              <button
                ...
                className={`w-full rounded-2xl border bg-white p-5 text-left shadow-soft transition-all ${
                  notif.isRead
                    ? "border-outline opacity-75"
                    : "border-primary/20 ring-1 ring-primary/10 hover:border-primary/40 hover:shadow-card"
                }`}
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl ${notif.isRead ? "bg-surface-variant text-on-surface-variant" : tone.box}`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p
                          className={`text-label-md font-bold ${notif.isRead ? "text-on-surface-variant" : "text-on-surface"}`}
                        >
                          {title}
                        </p>
                        {message && (
