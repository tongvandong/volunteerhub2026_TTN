...
export default function CertificateVerify() {
  ...
  const verify = async (event) => {
    ...
    try {
      ...
      setError(
        getErrorMessage(
          err,
          "Không tìm thấy hoặc không xác thực được chứng nhận",
        ),
      );
    } finally {
      setLoading(false);
  ...
  const certCode =
    certificate?.code || certificate?.certificateCode || code.trim();
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <form
        onSubmit={verify}
        className="bg-white rounded-3xl shadow-soft border border-outline p-6 flex flex-col md:flex-row gap-3"
      >
        <input
        ...
        <div className="bg-white rounded-[2rem] shadow-soft border border-outline overflow-hidden">
          <div className="bg-gradient-to-r from-primary to-tertiary p-8 text-white">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="uppercase tracking-[0.3em] text-white/80 text-label-sm font-bold">
                  VolunteerHub Certificate
                </p>
                <h2 className="text-display-sm font-black mt-1">
                  Chứng nhận hợp lệ
                </h2>
              </div>
            </div>
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Info
              label="Trạng thái"
              value={
                <StatusBadge status={certificate.status || "Verified"}>
                  Đã xác thực
                </StatusBadge>
              }
            />
            ...
            <Info
              label="Ngày cấp"
              value={formatDate(certificate.issuedAt || certificate.createdAt)}
            />
            <Info
              label="Số giờ ghi nhận"
              value={`${certificate.hours || certificate.totalHours || certificate.participationHours || 0} giờ`}
            />
            <div className="md:col-span-2">
              <Info
                ...
                value={
                  ...
                  certificate.note ||
                  certificate.summary ||
                  "Chứng nhận được phát hành bởi VolunteerHub."
                }
              />
            </div>
            <div className="md:col-span-2 flex flex-wrap gap-3 pt-2">
              <a
                ...
                href={certificateApi.getPdfUrl(certCode)}
                target="_blank"
                rel="noreferrer"
              >
                Tải PDF
...
function Info({ label, value }) {
  return (
    <div className="rounded-2xl bg-surface-variant/40 border border-outline p-4">
      ...
      <div className="mt-2 text-title-md font-bold text-on-surface">
        {value}
      </div>
    </div>
  );
