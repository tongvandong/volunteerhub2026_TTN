...
const getTitle = (tab, item) => {
  ...
  if (tab === "skill")
    return item.volunteerName || item.userName || "Volunteer";
...
const getSubtitle = (tab, item) => {
  ...
  if (tab === "skill")
    return [item.skillName, item.level].filter(Boolean).join(" · ");
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <DetailRow
            label="Ngày gửi"
            value={formatDateTime(item.verificationSubmittedAt)}
          />
        </div>
        <DetailRow
          label="Ghi chú của volunteer"
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <DetailRow
          label="Ngày gửi"
          value={formatDateTime(item.kycSubmittedAt)}
        />
      </div>
      <div className="flex flex-wrap gap-2">
...
export default function AdminVerification() {
  ...
  const fetchData = async (tab = activeTab) => {
    ...
    try {
      const res =
        tab === "kyc"
          ? await adminApi.getVolunteerKycRequests({
              status: "PendingVerification",
            })
          : tab === "skill"
            ? await adminApi.getVolunteerSkillVerifications({
                status: "PendingVerification",
              })
            : await adminApi.getOrganizerVerifications({
                status: "PendingVerification",
              });
      ...
      setItems([]);
      setError(
        getErrorMessage(err, "Không tải được danh sách yêu cầu xác minh."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const runReview = async (item, action) => {
    ...
    try {
      if (activeTab === "kyc") {
        if (action === "approve")
          await adminApi.approveVolunteerKyc(id, { note: "Approved by admin" });
        else await adminApi.rejectVolunteerKyc(id, { note: note.trim() });
      } else if (activeTab === "skill") {
        if (action === "approve")
          await adminApi.approveVolunteerSkill(id, {
            note: "Approved by admin",
          });
        else await adminApi.rejectVolunteerSkill(id, { note: note.trim() });
      } else if (action === "requestChanges") {
        await adminApi.requestOrganizerVerificationChanges(id, {
          note: note.trim(),
        });
      } else if (action === "approve") {
        await adminApi.approveOrganizerVerification(id, {
          note: "Approved by admin",
        });
      } else {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-on-surface-variant">
          Duyệt hồ sơ danh tính, kỹ năng và pháp lý tổ chức.
        </p>
      </div>
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-2 rounded-xl px-5 py-2.5 text-label-md font-bold transition-all 
              ${
                activeTab === tab.key
                  ? "border border-outline bg-white text-primary shadow-sm"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
          {items.map((item) => (
            <div
              key={item.id}
              className="rounded-3xl border border-outline bg-white p-6 shadow-soft"
            >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-on-surface">
                      {getTitle(activeTab, item)}
                    </h4>
                    <p className="text-label-sm text-on-surface-variant">
                      {getSubtitle(activeTab, item)}
                    </p>
                  </div>
                </div>
                <StatusBadge status={getStatus(activeTab, item)}>
                  {getStatus(activeTab, item) || "Chờ duyệt"}
                </StatusBadge>
              </div>
