...
export default function AdminRatings() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await ratingApi.getAdminRatings({
        page,
        pageSize: PAGE_SIZE,
  ...
  const action = async (type) => {
    ...
    try {
      if (type === "hide")
        await ratingApi.hide(selected.id, reason || "Ẩn bởi quản trị viên");
  ...
  const openEdit = (item) => {
    setSelected(item);
    setEdit({
      score: item.score || item.rating || 5,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Lọc, sửa, ẩn/hiện và xóa đánh giá hai chiều."
      />
      ...
      <div className="bg-white rounded-3xl shadow-soft border border-outline p-5">
        <div className="grid md:grid-cols-[1fr_220px] gap-3 mb-5">
          <SearchInput
            ...
            placeholder="Tìm theo người dùng, sự kiện..."
          />
          <select
        ...
        {loading ? (
          ...
          <EmptyState icon="reviews" title="Chưa có đánh giá" />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px]">
              <thead className="bg-surface-variant/50">
                <tr>
                  <th className="text-left p-4 text-label-sm uppercase tracking-wider text-on-surface-variant">
                    Sự kiện
                  </th>
                  <th className="text-left p-4 text-label-sm uppercase tracking-wider text-on-surface-variant">
                    Người đánh giá
                  ...
                  <th className="text-right p-4 text-label-sm uppercase tracking-wider text-on-surface-variant">
                    Thao tác
                  </th>
                </tr>
              </thead>
              ...
              <tbody>
                {ratings.map((item) => (
                  <tr
                    key={item.id}
                    className="border-t border-outline hover:bg-primary-container/10"
                  >
                    <td className="p-4 font-bold text-on-surface">
                      {item.eventTitle || item.event?.title || "—"}
                    </td>
                    <td className="p-4 text-body-sm text-on-surface-variant">
                      {item.raterName || item.rater?.name || "—"} →{" "}
                      {item.rateeName || item.ratee?.name || "—"}
                    </td>
                    <td className="p-4">
                      <StarRating value={item.score || item.rating} readOnly />
                    </td>
                    <td className="p-4 text-body-sm text-on-surface-variant max-w-xs">
                      {item.comment || "—"}
                    </td>
                    <td className="p-4">
                      <StatusBadge
        ...
        <div className="fixed inset-0 z-[80] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-soft border border-outline w-full max-w-lg p-6">
            ...
            <div className="mt-4">
              <StarRating
                value={edit.score}
                onChange={(score) => setEdit((p) => ({ ...p, score }))}
              />
            </div>
            <textarea
              ...
              value={edit.comment}
              onChange={(e) =>
                setEdit((p) => ({ ...p, comment: e.target.value }))
              }
            />
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button className="btn-primary" onClick={() => action("update")}>
                Lưu
              </button>
            </div>
          </div>
