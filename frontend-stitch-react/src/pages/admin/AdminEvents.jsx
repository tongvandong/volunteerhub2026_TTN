    ...
    try {
      ...
      if (action === "cancel") {
        if ((reason || "").trim().length < 10)
          throw new Error("Lý do hủy cần tối thiểu 10 ký tự");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-soft border border-outline">
          <p className="text-on-surface-variant text-label-md">Quá hạn xử lý</p>
          <p className="text-display-sm font-bold text-warning">
            {overdue.length}
          </p>
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-soft border border-outline">
        ...
        <div className="bg-warning-container/60 rounded-3xl border border-warning/20 p-5">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {overdue.slice(0, 4).map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-2xl p-4 border border-outline"
              >
                <p className="font-bold text-on-surface">
                  Sự kiện: {item.title || item.name}
                </p>
            ...
            <table className="w-full min-w-[980px]">
              ...
              <tbody>
                {events.map((event) => (
                  <tr
                    key={event.id}
                    className="border-t border-outline hover:bg-primary-container transition-colors"
                  >
                    <td className="p-4">
                      <p className="font-bold text-on-surface">
                    ...
                    <td className="p-4">
                      <StatusBadge status={event.status}>
                        {EVENT_STATUS_LABELS[event.status] || "Chưa cập nhật"}
                      </StatusBadge>
                    </td>
                    <td className="p-4">
