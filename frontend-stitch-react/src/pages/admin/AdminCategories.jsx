  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="CRUD danh mục sự kiện, kỹ năng hệ thống. " //và generic categories.
      />
      {error && <Alert type="error">{error}</Alert>}
      <TabBar
        tabs={Object.entries(modules).map(([value, cfg]) => ({
          value,
          label: cfg.label,
        }))}
        active={active}
      ...
      <div className="grid lg:grid-cols-[380px_1fr] gap-6">
        <form
          onSubmit={submit}
          className="bg-white rounded-3xl shadow-soft border border-outline p-6 space-y-4"
        >
          <h2 className="text-title-md font-bold text-on-surface">
            {editing ? "Cập nhật" : "Tạo mới"} {current.label}
          </h2>
          <div>
        ...
        <div className="bg-white rounded-3xl shadow-soft border border-outline p-5">
          {loading ? (
            ...
            <EmptyState icon={current.icon} title="Chưa có dữ liệu" />
          ) : (
