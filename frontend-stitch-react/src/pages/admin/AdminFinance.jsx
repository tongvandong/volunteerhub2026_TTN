...
export default function AdminFinance() {
  ...
  const chartData = useMemo(() => {
    ...
    const pendingDonations = safeNumber(
      finance?.pendingDonationAmount ?? finance?.pendingDonations,
    );
    ...
    return [
      {
        label: "Quyên góp đã xác nhận",
        value: confirmedDonations,
        color: "bg-primary",
        icon: "volunteer_activism",
      },
      {
        label: "Tài trợ đã ghi nhận",
        value: totalSponsorships,
        color: "bg-tertiary",
        icon: "handshake",
      },
      {
        label: "Quyên góp chờ xác nhận",
  ...
  const totalFlow =
    safeNumber(finance?.totalDonations) +
    safeNumber(finance?.totalSponsorships);
  const pendingRate =
    totalFlow > 0
      ? Math.round(
          (safeNumber(
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-on-surface-variant">
            Theo dõi quyên góp, tài trợ và dòng tiền trong hệ thống.
          </p>
        </div>
        <button
          onClick={() => adminApi.exportFinance("csv")}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <MetricCard
          ...
          value={formatMoney(finance?.totalDonations)}
          icon="volunteer_activism"
        />
        <MetricCard
          label="Tổng tài trợ"
          value={formatMoney(finance?.totalSponsorships)}
          icon="handshake"
        />
        <MetricCard
          label="Chiến dịch đang mở"
          value={finance?.openCampaigns || 0}
          icon="campaign"
          highlight="text-primary"
        />
        <MetricCard
          label="Chờ xác nhận"
          value={finance?.pendingDonations || 0}
          icon="pending_actions"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.3fr_0.7fr] gap-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-8">
            <div>
              ...
              <p className="text-on-surface-variant text-body-sm">
                Biểu đồ thanh thể hiện quy mô các nguồn tiền chính.
              </p>
            </div>
        ...
        <div className="bg-white rounded-3xl p-8 shadow-soft border border-outline">
          ...
          <div className="relative w-56 h-56 mx-auto rounded-full bg-[conic-gradient(var(--tw-gradient-stops))] from-primary from-0% via-primary to-warning flex items-center justify-center">
            {/* Vẽ vòng tròn/ biểu đồ tròn */}
            <div
              className="absolute inset-0 rounded-full"
              style={{
                background: `conic-gradient(#3b82f6 0 ${completedRate}%, #f59e0b ${completedRate}% 100%)`,
              }}
            />
