...
namespace BaseCore.Common
{
    public static class RedisUtils
    {
        public static async void DeleteAllCacheAsyn(this IConnectionMultiplexer muxer, IConfiguration configuration, string pattern)
        {
            ...
            var configuredDatabase = configuration["Redis:DefaultDatabase"];
            var defaultDatabase = !string.IsNullOrEmpty(configuredDatabase)
                ? int.Parse(configuredDatabase) : 10;
            var connectionString = configuration["Redis:ConnectionString"];
            if (string.IsNullOrWhiteSpace(connectionString)) return;
            var server = muxer.GetServer(connectionString);
            ...
            where TResult : class
        {
            ...
            if (storingValue != null && storingValue != default(TResult))
            {
                ...
                if (!value.Equals("[]")) //value is empty no cache
                {
                    ...
                    {
                        var configuredExpireMinute = configuration["Redis:CachingExpireMinute"];
                        var expireMinute = !string.IsNullOrEmpty(configuredExpireMinute) ? int.Parse(configuredExpireMinute) : 1;
