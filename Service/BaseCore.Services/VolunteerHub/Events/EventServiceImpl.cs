...
namespace BaseCore.Services.VolunteerHub.Events
{
    public class EventServiceImpl : IEventService
    {
        ...
        public EventServiceImpl(MySqlDbContext context, INotificationService notificationService, ICertificateService certificateService)
        {
            _context = context;
        ...
        private async Task ApplyConfirmedParticipantCountsAsync(IEnumerable<BaseCore.Entities.Event> events)
        {
        ...
        public async Task<BaseCore.Entities.Event> CompleteAsync(int eventId, int? organizerId = null, IReadOnlyCollection<ManualCompletionAttendance>? manualAttendances = null)
        {
            var ev = await _context.Events.FindAsync(eventId)
                ?? throw new Exception("Event not found");
